...
public class LeaderController {
    ...
    public Result findAllAViProjectCategory() {
        return leaderService.AllAviProject();
    }
    ...
    @GetMapping("/ProjectByLeader")
    @ApiOperation(value = "领导查看所有负责的项目")
    public Result getProjectByLeader(){
        String leaderId = UserContext.getCurrentUser().getUserId();
        return leaderService.ProjectByLeader(leaderId);
    }

    @GetMapping("/MoreInfo")
    @ApiOperation(value = "查看用户一个项目的详细信息")
    public Result findProjectMoreInfo(@RequestParam(value = "applicationId") String applicationId) {
        return leaderService.MoreInfo(applicationId);
    }
}
